...
Run with `./sagew experiments/aft/stepstone/<script>`.
...
Run with `./sagew experiments/aft/stepstone/profiles/<script>`.
...
Run with `./sagew experiments/aft/stepstone/fractal/<script>`.
...
coastline_series.sage (shared, at experiments/ root)
  +-- ripple/stability_heatmap.sage  --> ripple/results/
  +-- ripple/settlers.sage           --> ripple/results/
  +-- ripple/divergent.sage          --> ripple/results/
```

### `ripple/` subfolder

Coastline area across partition families. Scaled-area convergence
and stability diagnostics. See [`ripple/RIPPLE.md`](ripple/RIPPLE.md).
...
`ln(2) / (8 N^2)`. This is visible in the cartesian envelope as a flat
peak line. No other partition kind achieves this.
