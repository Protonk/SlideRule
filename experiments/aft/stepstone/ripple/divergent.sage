"""
divergent.sage — Sparklines for partitions whose scaled coastline area
diverges or never settles, showing three distinct failure modes.

beta/stern-brocot/cantor have near-constant positive log-ratios (exponential
growth). farey-rank grows irregularly. golden oscillates with persistent
±0.04 wobble that never damps. random is the stochastic null model.
bitrev-geometric is the bridge — the slowest settler, providing scale context.

Run:  ./sagew experiments/aft/stepstone/ripple/divergent.sage
"""
...
OUT_PATH = pathing('experiments', 'aft', 'stepstone', 'ripple', 'results', 'divergent.png')
...
def precompute():
    ...
    # Always include geometric for the measure context (geo-referenced measures).
    if 'geometric_x' not in kinds:
        kinds = kinds + ['geometric_x']
    print()
...
def make_plot(plot_Ns, measured, meta):
    ...
    fig.suptitle(
        'The divergent and the restless: unbounded or oscillatory coastline\n'
