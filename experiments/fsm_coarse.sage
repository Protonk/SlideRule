...
Run from project root:  sage experiments/fsm_coarse.sage
"""
...
def run_experiment(q, depth, p, qpow):
    """
...
def print_row(result):
    ...
    print(f"  q={result['q']:>2}  d={result['depth']:>2}  "
...
def validate(q, depth, p, qpow, nsamp=2048):
...
def main():
    ...
    for q_v, d_v, p_v, qp_v in [(1,4,1,2), (3,5,1,2), (5,4,2,3), (2,6,1,3)]:
        disc, checked = validate(q_v, d_v, p_v, qp_v)
        print(f"  q={q_v} depth={d_v} alpha={p_v}/{qp_v}  "
    ...
    for q in (1, 2, 3, 5):
        for depth in (3, 4, 5, 6, 7, 8):
            if 2**depth > 512 and q > 3:
                continue
            result = run_experiment(q, depth, p=1, qpow=2)
            print_row(result)
        print()
    ...
    for q in range(1, 10):
        result = run_experiment(q, depth=6, p=1, qpow=2)
        print_row(result)
    print()
    ...
    for p, qpow in alphas:
        result = run_experiment(q=3, depth=6, p=p, qpow=qpow)
        print_row(result)
    print()
    ...
    print()
    print("Key question: does sidon_sub or cf grow nontrivially")
    print("as q and/or depth increase?")
    print("=" * 80)
