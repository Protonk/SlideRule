...
organizes everything. Steps 1–3 of TRAVERSE build on this
cleanly. But the FSM — the architecture we chose to *correct* the
surrogate — does not share the symmetry it is correcting for, and
Step 4 can no longer be treated as if it had already escaped that
fact.
...
The architecture-dependence result ([DEPENDENCE](../experiments/wall/DEPENDENCE.md),
2026-03-26) sharpens the point. Inside the canonical mod-q family,
more states can produce a *larger* wall at fixed depth because the
achievable subspace changes orientation with q. The mod-q sweep is
therefore not a pure capacity axis. It is a family of different
automata.

This matters for the roadmap because:

**Step 4 is an FSM exchange-rate step, not a universal one.** If the
dominant wall source is a topological mismatch between the correction
architecture and the function's symmetry, and if the mod-q sweep moves
through non-nested subspaces, then the `(C, gap)` curve is measuring
how this architecture spends structure. It is not yet measuring the
cost of approximation in general. Tracked as the 
[AUTOMATON-SARGASSUM](AUTOMATON-SARGASSUM.md).

**Step 5 becomes a filter.** Any apparent
ordering extracted from the canonical FSM sweep must survive hostile
tests — partition surgery, structured null families, and eventually a
second binary-digit-reading architecture — before it can be promoted
toward architecture-invariant status. Tracked at the 
...
they lead to different versions of the route, and the safe route is
now clear: characterize the FSM as an architecture first, then ask
what survives its deformations, and only then try to cross into
computational language.

### What would resolve it

1. **FSM-managed characterization.** Fan-out scaling data and
   topology-sensitivity data. If the layer-0 displacement range
   ...
   in principle absorb it. If it grows, the cost is structural and
   permanent. Likewise, if matched-cost topology changes produce large
   wall spreads, the architecture question is genuinely load-bearing.

2. **Invariance filters.** First the adversarial zoo-surgery test,
   then structured null families that preserve binary-address features
   while randomising others, and then a second binary-representation
   architecture. Not a log-equivariant one (that's circular), but
   another architecture that also processes binary digits and shares
   structure across cells — such as De Caro's shared-coefficient
   piecewise polynomials. If the relevant ordering or frontier
   survives those filters, it has earned promotion toward
   architecture-invariant status.

3. **A lower bound.** A minimax lower bound on shared-structure
   approximation of ε from binary representations, matching the
   observed wall magnitude. This would prove the wall is
   representation-intrinsic regardless of architecture and would make
   the crossing to Complexity Reef legitimate.
